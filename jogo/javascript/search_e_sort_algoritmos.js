 ...
 */
export function binarySearch(arrayOrdenado, alvo) {
    let esquerda = 0;
...
export function testSort(name, sortFunc, data) {
    const startTime = performance.now();
    const sorted = sortFunc(data);
    const endTime = performance.now();
    return endTime - startTime;
}
export function testSearch(name, searchFunc, data, target) {
    const startTime = performance.now();
    const search = searchFunc(data, target);
    const endTime = performance.now();
    return endTime - startTime;
}
//# sourceMappingURL=search_e_sort_algoritmos.js.map